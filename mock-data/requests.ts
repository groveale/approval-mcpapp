 ...
 * Production: uses Managed Identity (AZURE_STORAGE_ACCOUNT_NAME env var).
 * Development: falls back to a connection string (Azurite by default).
 */
...
function makeTableClient(tableName: string): TableClient {
  ...
  return TableClient.fromConnectionString(DEV_CONN, tableName, { allowInsecureConnection: true });
}

function makeServiceClient(): TableServiceClient {
  if (STORAGE_ACCOUNT) {
  ...
  return TableServiceClient.fromConnectionString(DEV_CONN, { allowInsecureConnection: true });
}

const requestsTable = makeTableClient("AccessRequests");
const countersTable = makeTableClient("Counters");

// Ensure tables exist in both local and Azure deployments.
// The storage account is provisioned by Bicep, but the tables themselves are not.
const tablesReady = (async () => {
  const svc = makeServiceClient();
  for (const tableName of ["AccessRequests", "Counters"]) {
    try {
      await svc.createTable(tableName);
    } catch (error) {
      if (isTableAlreadyExistsError(error)) continue;

      console.error(`Failed to ensure Azure Table Storage table ${tableName}`, error);
      throw error;
    }
  }
})();
